...
import type {
  CustomTableColumn,
  CustomTableColumnDataType,
  CustomTableRow,
  CustomTableCellValue,
  SaveCustomTableColumnPayload,
 ...
 * modal input, etc.) — kept exported so a page's own page-shell markup (the
 * project dropdown, tab bar) matches this component's own inputs exactly. */
...
const COLUMN_DATA_TYPES: { value: CustomTableColumnDataType; label: string }[] = [
...
// ---- Add / rename table modal — used by a page's own tab bar, not TableSheet itself ----

export const TableNameModal: React.FC<{
  title: string;
  initialName?: string;
  confirmLabel: string;
...
const AddColumnModal: React.FC<{
  ...
  onClose: () => void;
}> = ({ onCreateColumn, onClose }) => {
  const [name, setName] = useState("");
  const [dataType, setDataType] = useState<CustomTableColumnDataType>("text");
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onCreateColumn({ name: name.trim(), dataType, target: target.trim() ? Number(target) : null });
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="w-full max-w-sm overflow-hidden bg-white border rounded-xl shadow-2xl border-slate-200/70">
        ...
        <div className="p-4 space-y-2">
          ...
          <select className={inputCls} value={dataType} onChange={(e) => setDataType(e.target.value as CustomTableColumnDataType)}>
...
const EditColumnModal: React.FC<{
  ...
  /** True only when this is the last Date column of a table whose structure
   * is locked (e.g. Plant Report's auto-seeded default table) — blocks
   * changing its type away from Date or deleting it. */
  isLastDateColumnLocked: boolean;
  onUpdateColumn: (id: number, payload: SaveCustomTableColumnPayload) => Promise<CustomTableColumn>;
  onDeleteColumn: (id: number) => Promise<void>;
  onClose: () => void;
}> = ({ column, isLastDateColumnLocked, onUpdateColumn, onDeleteColumn, onClose }) => {
  const [name, setName] = useState(column.name);
  const [dataType, setDataType] = useState<CustomTableColumnDataType>(column.dataType);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onUpdateColumn(column.id, { name: name.trim(), dataType, target: target.trim() ? Number(target) : null });
      onClose();
    } catch (err) {
        ...
        <div className="w-full max-w-sm overflow-hidden bg-white border rounded-xl shadow-2xl border-slate-200/70">
          ...
          <div className="p-4 space-y-2">
            ...
            <select
              className={inputCls}
              value={dataType}
              onChange={(e) => setDataType(e.target.value as CustomTableColumnDataType)}
              disabled={isLastDateColumnLocked}
            >
              {COLUMN_DATA_TYPES.map((t) => (
                <option key={t.value} value={t.value}>
                  {t.label}
            ...
            {isLastDateColumnLocked && (
              <p className="text-[11.5px] text-slate-400">This tab must keep at least one Date column.</p>
            )}
            {error && <p className="text-[11.5px] text-red-600">{error}</p>}
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={isLastDateColumnLocked}
                title={isLastDateColumnLocked ? "This tab must keep at least one Date column" : undefined}
...
const Cell: React.FC<{
  ...
  onCommit: (value: CustomTableCellValue) => void;
  editable: boolean;
...
function coerceToColumnType(raw: unknown, dataType: CustomTableColumnDataType, dateFormat: "AD" | "BS" = "AD"): CustomTableCellValue {
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {previewRows.map((row, i) => (
                    <tr key={i} className={`border-b border-slate-100 last:border-0 ${i % 2 === 1 ? "bg-slate-200/60" : "bg-white"}`}>
                      {pending.matchedColumns.map((col) => (
...
// ---- Export — download the current table as an .xlsx, one column per CustomTableColumn ----

const formatCellForExport = (value: CustomTableCellValue, dataType: CustomTableColumnDataType): string | number | boolean | null => {
  if (value == null) return null;
  if (dataType === "boolean") return value ? "Yes" : "No";
  return value;
};

const exportTableToXlsx = (tableName: string, columns: CustomTableColumn[], rows: CustomTableRow[]) => {
...
const ExportSheetButton: React.FC<{ tableName: string; columns: CustomTableColumn[]; rows: CustomTableRow[] }> = ({
  tableName,
  columns,
  rows,
}) => (
...
export function getPeriodKey(date: Date, granularity: Exclude<Granularity, "daily">): { key: string; label: string } {
...
function aggregateRows(
  rows: CustomTableRow[],
  dateColumn: CustomTableColumn,
  numberColumns: CustomTableColumn[],
...
const AggregatedTable: React.FC<{ granularity: Granularity; rows: AggregatedRow[]; numberColumns: CustomTableColumn[] }> = ({
  granularity,
  rows,
  numberColumns,
}) => (
      ...
      <tbody>
        {rows.length === 0 ? (
          ...
          rows.map((r, i) => (
            <tr
              key={r.key}
              className={`border-b border-slate-100 last:border-0 hover:bg-blue-50/60 ${i % 2 === 1 ? "bg-slate-200/60" : "bg-white"}`}
            >
              <td className="px-3 py-2.5 text-[12.5px] font-medium text-black whitespace-nowrap">{r.label}</td>
              <td className="px-3 py-2.5 text-[12.5px] text-right text-black">{r.count}</td>
              {numberColumns.map((c) => (
                <td key={c.id} className="px-3 py-2.5 text-[12.5px] text-right text-black">
...
export interface TableSheetProps {
  ...
  isAdmin: boolean;
  tableName: string;
  /** True only for a table whose structure can't be freely changed (Plant
   * Report's auto-seeded default table) — blocks removing/retyping its last
   * Date column. Defaults to false (every Materials tab). */
  locked?: boolean;
  detail: { columns: CustomTableColumn[]; rows: CustomTableRow[] } | undefined;
  isLoading: boolean;
  isError: boolean;
  errorMessage?: string;
  onCreateColumn: (payload: SaveCustomTableColumnPayload) => Promise<CustomTableColumn>;
  onUpdateColumn: (id: number, payload: SaveCustomTableColumnPayload) => Promise<CustomTableColumn>;
  onDeleteColumn: (id: number) => Promise<void>;
  onCreateRow: (payload: SaveCustomTableRowPayload) => Promise<CustomTableRow>;
 ...
 * — column/row CRUD, cell editing, XLSX import/export, and a daily/weekly/
 * monthly aggregation view. Purely presentational: the container page owns
 * the react-query hooks and passes data + these CRUD callbacks in as props. */
export const TableSheet: React.FC<TableSheetProps> = ({
  ...
  const [confirmDeleteRow, setConfirmDeleteRow] = useState<CustomTableRow | null>(null);
  ...
  const dateColumns = useMemo(() => (detail?.columns ?? []).filter((c) => c.dataType === "date"), [detail]);
  const numberColumns = useMemo(() => (detail?.columns ?? []).filter((c) => c.dataType === "number"), [detail]);
  ...
  if (isError || !detail) {
    return <ErrorBanner message={errorMessage || "Failed to load table."} className="m-6" />;
  }
  ...
  const commitCell = (row: CustomTableRow, column: CustomTableColumn, value: CustomTableCellValue) => {
    ...
    onUpdateRow(row.id, { values: nextValues }).catch((err) => setActionError(getErrorMessage(err, "Failed to save cell.")));
  };
  ...
  const addRow = async () => {
    ...
    try {
      ...
      setActionError(getErrorMessage(err, "Failed to add row."));
    } finally {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(Array.from(selectedRowIds).map((id) => onDeleteRow(id)));
  ...
  return (
    <div className="px-6 py-5">
      ...
      {granularity !== "daily" ? (
        ...
        <div className="overflow-x-auto bg-white border rounded-xl shadow-sm border-slate-200">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/60">
                ...
                {columns.map((col) => (
                  <th key={col.id} className="py-2.5 px-3 text-[11px] font-medium text-blue-900 uppercase tracking-wide whitespace-nowrap">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((row, rowIndex) => (
                  <tr
                    key={row.id}
                    className={`border-b border-slate-100 last:border-0 hover:bg-blue-50/60 ${
                      selectedRowIds.has(row.id) ? "bg-blue-50/40" : rowIndex % 2 === 1 ? "bg-slate-200/60" : "bg-white"
                    }`}
                  >
          ...
          <div className="border-t border-slate-100">
            ...
            >
              {isAddingRow ? <Loader2 size={13} className="animate-spin" /> : <Plus size={13} />}
              Add Row
            </button>
          </div>
        </div>
      ...
      {addColumnOpen && <AddColumnModal onCreateColumn={onCreateColumn} onClose={() => setAddColumnOpen(false)} />}
      {editingColumn && (
        <EditColumnModal
          column={editingColumn}
          isLastDateColumnLocked={locked && editingColumn.dataType === "date" && dateColumns.length <= 1}
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          ...
          try {
            await onDeleteRow(confirmDeleteRow.id);
