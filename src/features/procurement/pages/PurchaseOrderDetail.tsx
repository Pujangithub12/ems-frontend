...
import {
  ...
  useUploadProformaInvoiceFileMutation,
} from "../hooks/useProformaInvoice";
import {
...
const tabs = [
  ...
  { id: "pi", label: "Proforma Invoice", icon: FileText },
 ...
 * five data-rich tabs (Overview, Proforma Invoice, Shipment, Cost Sheet, Goods
 * Receipt). One usePurchaseOrderDetailQuery covers everything except the Cost
 * Sheet, which is its own computed-on-the-fly endpoint. Every mutation across
 * every tab refetches both queries afterward — this codebase's mutations never
...
const PurchaseOrderDetailPage: React.FC = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "pi":
        return <ProformaInvoiceTab po={po} isAdmin={isAdmin} onChanged={refetchAll} />;
      case "shipment":
    ...
    <div className="flex flex-col gap-4 max-w-5xl">
      ...
      <div className={sectionCardCls}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[13px] font-semibold text-slate-900">Order Details</h3>
          {isAdmin && (
...
const ProformaInvoiceTab: React.FC<{ po: PurchaseOrder; isAdmin: boolean; onChanged: () => Promise<void> }> = ({ po, isAdmin, onChanged }) => {
  ...
  const proformaInvoices = po.proformaInvoices ?? [];
    ...
    <div className="flex flex-col gap-4 max-w-5xl">
      ...
      {proformaInvoices.length === 0 ? (
        <div className={`${sectionCardCls} text-center py-8`}>
          <p className="text-[12px] text-slate-400">No proforma invoices yet.</p>
        </div>
      ) : (
        proformaInvoices.map((pi: ProformaInvoice) => (
              ...
              <div className="flex items-center gap-2">
                  ...
                  <label className="flex items-center gap-1 text-[11px] font-medium text-blue-700 cursor-pointer hover:underline">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        e.target.value = "";
                        if (!file) return;
                        await runAction(() => uploadFileMutation.mutateAsync({ id: pi.id, file }));
                      }}
                    />
                  </label>
                ) : null}
                {isAdmin && pi.status === "waiting" && (
                  <>
                    <button
                      disabled={busy}
                      onClick={() => runAction(() => changeStatusMutation.mutateAsync({ id: pi.id, status: "approved" }))}
                    ...
                    <button
                      disabled={busy}
                      onClick={() => runAction(() => changeStatusMutation.mutateAsync({ id: pi.id, status: "rejected" }))}
      ...
      {isAdmin && (
        <div className={sectionCardCls}>
          <div className="flex items-center justify-between">
            <h3 className="text-[13px] font-semibold text-slate-900">Add Proforma Invoice</h3>
            <button onClick={() => setShowForm((s) => !s)} className="flex items-center gap-1 text-[11px] font-medium text-blue-700 hover:underline">
              <Plus size={12} /> {showForm ? "Cancel" : "New Proforma Invoice"}
            </button>
          </div>
          {showForm && (
